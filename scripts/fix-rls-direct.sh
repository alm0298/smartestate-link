...
# This script fixes RLS policies by connecting directly to the Supabase database

echo "Running SQL commands to fix RLS policies..."
...
# Run the SQL commands using the Supabase CLI
echo "Attempting to run SQL commands using Supabase CLI..."
supabase db remote psql --file $TMP_SQL_FILE

# Check if the command was successful
if [ $? -eq 0 ]; then
  echo "RLS policies have been fixed successfully!"
else
  echo "Failed to run SQL commands using Supabase CLI."
  echo "Please run the following SQL commands directly in the Supabase dashboard SQL editor:"
  cat $TMP_SQL_FILE
fi
