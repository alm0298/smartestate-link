...
`;
  ...
  // Test the connection
  console.log('\nWould you like to test the Supabase connection?');
  const testConnection = await prompt('Test connection? (y/n): ');
  ...
  if (testConnection.toLowerCase() === 'y') {
    try {
      ...
      const { data, error } = await supabase.from('property_analyses').select('count');
      ...
      console.log('You may need to install @supabase/supabase-js if it\'s not already installed:');
