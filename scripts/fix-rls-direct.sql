-- STEP 1: Check if RLS is enabled on property_analyses
SELECT 
...
-- STEP 2: Enable RLS if it's not already enabled
...
-- STEP 4: Create the property_shares table if it doesn't exist
...
-- STEP 5: Enable RLS on property_shares table
ALTER TABLE property_shares ENABLE ROW LEVEL SECURITY;

-- STEP 6: Drop all existing policies on property_shares
...
-- STEP 7: Create simplified policies for property_analyses
...
-- STEP 8: Create simplified policies for property_shares
...
-- STEP 10: Check if there are properties in the database
SELECT COUNT(*) AS total_properties FROM property_analyses;

-- STEP 11: Check if there are property shares in the database
